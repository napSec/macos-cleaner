...
echo "Enter the numbers of the directories to search (separated by 
spaces):"
read -r selection
...
if [[ -z "$selection" ]]; then
    ...
    # Build an array of directories to search based on the selected 
numbers
    search_directories=()
...
# Find all files in the selected directories, and compute their SHA-1 
hashes
find "${search_directories[@]}" -type f -print0 | xargs -0 shasum -a 1 | 
sort > /tmp/duplicates.txt

# Extract the list of duplicate files based on their SHA-1 hashes
cat /tmp/duplicates.txt | uniq -d | cut -f 1 -d ' ' | xargs -I{} grep {} 
/tmp/duplicates.txt > /tmp/duplicate_files.txt
